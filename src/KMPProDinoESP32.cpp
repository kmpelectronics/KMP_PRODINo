...
// Status RGB LED.
const uint16_t StatusLedPixelCount = 1;
const uint8_t StatusLedPin = 0;
const uint16_t StatusLedPixelNumber = 0;
NeoPixelBus<NeoGrbFeature, NeoEsp32BitBang800KbpsMethod> _pixel(StatusLedPixelCount, StatusLedPin);
...
{
	_pixel.SetPixelColor(StatusLedPixelNumber, 0);
	_pixel.Show();
}
