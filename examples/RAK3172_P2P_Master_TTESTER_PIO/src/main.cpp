...
bool readRemoteADCraw(uint8_t address)
{
  uint8_t txMessage[]={MASTER_ADDRESS,address,CMD_READ_ADC};
...
void readUartCmd()
{
  ...
  {
    uint8_t rxCMD[16];
    size_t cmdlen = Serial.readBytes(rxCMD, sizeof(rxCMD));

    if(!LoRa.transmitMessage(rxCMD, sizeof(cmdlen), rxMessage, sizeof(rxMessage), &msglen, DEFAULT_CMD_TIMEOUT))
    {
      uint8_t errorResponse[] = {0xAA,0xBB,0xCC,0xDD,0xEE,0xFF};
      Serial.write(errorResponse, sizeof(errorResponse));
    }
    else
    {
      Serial.write(rxMessage, msglen);
    }
